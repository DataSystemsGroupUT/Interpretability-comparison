...
def load_dataset(dataset_name, balance=False, discretize=True, dataset_folder='./'):
    ...
    if dataset_name == 'adult':
        ...
        feature_names = ["studycode","dos",'Age','race','Race','Sex','weight','height','bmi','target_heart_rt','METS','mets_c',
        				'resting_heart_rt','Resting Systolic Blood Pressure','resting_diastolic','peak_heart_rt','peak_systolic','peak_diastolic','% Heart Rate Achieved',
        				'adequatestudy85','maximalstudy','norm_rest_ekg_flg','Reason for test','amio','aspirin','bb','acei','arb','Diuretic','ccb','other_htnmed',
        				'Other Hypertension Medication','statin','other_lipid','all_lipid','insulin','dmmed','depressionmed','smokingmeds','plavix','nitrates','edmeds',
        				'dig','lungdiseasemeds','thyroidmeds','History of Hypertension','Diabetes Medication','hyperlipid','Family History of CHD','History of Smoking','ldl','hdl','chol','vldl','tg','cr',
        				'a1c','hscrp','Hemoglobin','gfr','dod2013','Risk of Mortality','t_to_dth','lastseen','Atrial Fibrillation','aflutter2','warfarin2','obesity2','sedentary2','FITDate','priorCVA']
...
def load_csv_dataset(data, target_idx, delimiter=',',
    ...
    #ret.ori_data = copy.deepcopy(data)
    ...
    if features_to_use is not None: 
        feats = copy.deepcopy(features_to_use)
        feats.append(target_idx)
        ret.ori_data = copy.deepcopy(data[:,feats])
        ...
        #ret.ori_data = ret.ori_data[:,features_to_use.append(target_idx)]
    ...
    if balance:
        ...
        data = data[idxs]
        ret.ori_data = ret.ori_data[idxs]
        labels = labels[idxs]
        ret.data = data
        ret.labels = labels
    ...
    ret.ori_train = ret.ori_data[train_idx]
    ...
    ret.ori_test = ret.ori_data[test_idx]
